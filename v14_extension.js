...
function marketRows(rows){
  if(!rows?.length)return '<p class="muted">No hay registros disponibles para este mercado en este momento.</p>';
  return `<div class="market-daily-head"><b>${rows.length} productos disponibles</b><small>Se muestra el último dato conocido cuando no existe actualización reciente</small></div>
  <table class="market-data"><thead><tr><th>Producto</th><th>Precio</th><th>Unidad</th><th>Ingreso</th><th>Mercado</th><th>Fecha</th><th>Estado</th></tr></thead><tbody>${rows.map(x=>{
    const st=x.status||'ÚLTIMO PRECIO DISPONIBLE';
    const cls=st==='ACTUALIZADO'?'market-status current':'market-status fallback';
    return `<tr><td><b>${esc(x.product)}</b></td><td>${x.price!=null?'S/ '+Number(x.price).toFixed(2):'—'}</td><td>${esc(x.unit||'kg')}</td><td>${x.ingressTons!=null?Number(x.ingressTons).toLocaleString('es-PE',{maximumFractionDigits:1})+' t':'—'}</td><td>${esc(x.market||x.zone||'—')}</td><td>${esc(x.date||'Sin fecha')}</td><td><span class="${cls}">${esc(st)}</span></td></tr>`;
  }).join('')}</tbody></table>`}
async function loadMarkets(){
  const zone=$('marketZone')?.value||'lima_gmml';
  $('marketTable').innerHTML='<p class="muted">Cargando reporte diario completo…</p>';
  try{
    const r=await fetch(`/api/markets?zone=${encodeURIComponent(zone)}`,{cache:'no-store'}),data=await r.json();
    if(!r.ok||!data.ok)throw Error(data.error||'No disponible');
    $('marketUpdated').textContent=data.reportDate?`Reporte: ${data.reportDate}`:(data.updatedAt?new Date(data.updatedAt).toLocaleString('es-PE'):'—');
    $('marketTable').innerHTML=`<div class="market-report-title"><strong>${esc(data.marketLabel||zone)}</strong><span>${data.rows?.length||0} registros</span></div>${marketRows(data.rows||[])}<div class="source-list">${(data.sources||[]).map(s=>`<a target="_blank" rel="noopener" href="${esc(s.url)}">${esc(s.name)} ↗</a>`).join('')}</div>`;
  }catch(e){$('marketTable').innerHTML='<p class="muted">El reporte diario de este mercado no está disponible en este momento.</p>'}
}
