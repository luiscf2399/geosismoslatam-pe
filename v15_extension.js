...
function hookFreightDisplay(){
  ...
  b.replaceWith(b.cloneNode(true));const nb=$('freightCalc');nb.addEventListener('click',async()=>{const origin=$('freightOrigin').value.trim(),destination=$('freightDestination').value.trim(),tons=+$('freightTons').value||0;if(!origin||!destination||tons<=0){$('freightResult').textContent='Completa origen, destino y toneladas.';return}$('freightResult').textContent='Calculando con referencias reales/publicadas…';try{const r=await fetch(`/api/freight?origin=${encodeURIComponent(origin)}&destination=${encodeURIComponent(destination)}&tons=${encodeURIComponent(tons)}`,{cache:'no-store'});const j=await r.json();if(!j.ok)throw Error();$('freightResult').innerHTML=`<b>${esc(origin)} → ${esc(destination)}</b><div class="freight-metrics"><span>Distancia <b>${Math.round(j.distanceKm)} km</b></span><span>Carga <b>${tons.toFixed(1)} t</b></span><span>Promedio <b>S/ ${Math.round(j.mid).toLocaleString('es-PE')}</b></span><span>Rango <b>S/ ${Math.round(j.low).toLocaleString('es-PE')} – S/ ${Math.round(j.high).toLocaleString('es-PE')}</b></span></div><small>${esc(j.note)}</small><details><summary>Referencias usadas</summary>${(j.basis||[]).map(x=>`<a target="_blank" rel="noopener" href="${esc(x.url)}">${esc(x.name)} ↗</a>`).join('<br>')}</details>`}catch{$('freightResult').textContent='No se pudo calcular el flete. Prueba con nombres más específicos.'}})
}
